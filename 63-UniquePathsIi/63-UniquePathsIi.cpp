// Last updated: 8/31/2026, 4:35:02 PM
class Solution1 {
public:
    int count(int i, int j, vector<vector<int>>& obstacleGrid){
        ...
        if(i==0 && j==0) return 1;
        if(i<0 || j<0) return 0;
        int up=count(i-1,j,obstacleGrid);
        int left=count(i,j-1,obstacleGrid);
        return up+left;
    }
    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        int n=obstacleGrid.size();
        int m=obstacleGrid[0].size();
        int cnt=count(n-1,m-1,obstacleGrid);
        return cnt;
    }
};
...
public:
    ...
    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        ...
        int cnt=count(n-1,m-1,obstacleGrid,dp);
        return cnt;
    }
};
